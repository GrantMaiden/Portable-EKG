...
boolean milliFlag = false;
boolean stopTimerFlag = false;
boolean startStabilize = true;
boolean whiteFlag = true;
boolean sdCardFull = false;
boolean menu = true;
boolean writeChoice = true;
boolean chose = false;
boolean playbackFlag = false;
boolean SDplaybackFlag = false;
boolean setupVariablesFlag = false;
...
void loop()
{
  ...
  if (!chose) {
    ...
    // Show recording
    if (playbackFlag || SDplaybackFlag) {
      readDataModule();
    }
    ...
    } else if (writeChoice && !playbackFlag) {
...
void chooseFile(int input) { // Controls and initializes choose file screen
  ...
  //  Serial.println(input);
  //  Serial.println("chooseFileCurrentNum:");
  //  Serial.println(chooseFileCurrentNum);
...
void displayECG() {
  ...
  /*
    if (millis() - hb_timer > 500) {
      ...
      tft.setRotation(3);
      //tft.fillScreen(ILI9341_BLACK);
      tft.fillRect(0, 0, 80, 40, ILI9341_BLACK);
      tft.setTextColor(ILI9341_BLUE);
      tft.setTextSize(2);
      tft.setCursor(0, 0);
      tft.println("BPM");
      tft.println((millis() - hb_timer) * 1000 / 60);
      tft.setRotation(1);
    }
  */
  if (trig) {
...
void playbackDraw() { // line creation and control part of readDataModule()
  ...
  if (SDidx <= 0 && PotentiometerValue <= 2200) {
    SDidx = 0;
  } else if (SDidx >= 7173 && PotentiometerValue >= 1900) {
    SDidx = 7173;
  }
  ...
  if (oldSDidx != SDidx || redrawFlag == false) {
    ...
    if ((1900 >= PotentiometerValue && PotentiometerValue > 2200) == false) {
      ...
      tft.fillCircle(10 + oldSDidx * 299 / 7173, 160, 5, ILI9341_WHITE);
      tft.fillCircle(10 + SDidx * 299 / 7173, 160, 5, ILI9341_BLUE);
      ...
      for (int j = 0; j < 319; j++) {
        ...
        oldPoint1 = recordSD[SDidx + j];
        oldPoint2 = recordSD[SDidx + j + 1];
      }
    }
  }
  tft.setRotation(1);
...
void adc0_isr() {
  ...
  if (tempTrig && writeChoice && !playbackFlag && chose) {
    ...
    for (int i = 319; i > 0; i--) { //create new line
      yPos[i] = yPos[i - 1];
      if ((140 < yPos[120] && yPos[120] < 220) && bool_heartBeatFlag == false && bool_hbTimerFlag == true) { // if QRS Peak is detected
        int j = 1;
        ...
        while (j < 20 && !PRI_detected) { // Begin pri detection : look for PRI dip
          ...
          if (PRI_detected == true)
            break;
          j = j + 1;
          if (yPos[120 + j + 1] > yPos[120 + j] && !PRI_detected) { // Q detected
            if (storeBeat == true) {
            ...
            while (j < 45) { // Look for P peak
              if (PRI_detected == true)
                break;
              j = j + 1;
              if (yPos[120 + j] > storeBeatVoltage && !PRI_detected) { // P peak detected
                j = j + 2;
                while (j < 60) {
                  if (PRI_detected == true)
                    break;
                  j = j + 1;
                  if (yPos[120 + j] < storeBeatVoltage) {
        ...
        //beatAvg = (yPos[i + 31] + yPos[i + 32] + yPos[i + 33] + yPos[i + 34] + yPos[i + 35] + yPos[i + 36] + yPos[i + 37] + yPos[i + 38] + yPos[i + 39] + yPos[i + 40] + yPos[i + 41]) / 11;
        if (PRI_detected && QT_detected) {
          if (hb_counter >= 9) {
            ...
            //Serial.println("bool_BRADY:");
            //Serial.println(bool_BRADY);
            //Serial.println("bool_TACHY:");
            //Serial.println(bool_TACHY);
            Serial.println("HB AVG:");
            Serial.println(HB_avg);
            ...
            //Serial.println("PRI_add_tally:");
            //Serial.println(PRI_add_tally2);
            //Serial.println("PRI_time:");
            //Serial.println(PRI_time);
            Serial.println("PRI_avg:");
            Serial.println(PRI_avg);
            //Serial.println("QRS_time:");
            //Serial.println(QRS_time);
            Serial.println("QRS_avg:");
            Serial.println(QRS_avg);
            if (PAC_counter < 7) {
              float HB_difference = ((float)previous_HB_length / (millis() - hb_timer));
              //Serial.println("HB_difference");
              //Serial.println(HB_difference);
            ...
            //Serial.println("PAC_counter:");
            //Serial.println(PAC_counter);
            //Serial.println("bool_PAC_detected:");
            //Serial.println(bool_PAC_detected);
          }
    ...
    //yPos[0] = 120 + 48 * (filterOneLowpass.input( analogRead(A6) ) - Baselineaverage) / 819;
